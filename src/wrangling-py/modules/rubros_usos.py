...
df = pd.read_csv(f"{DIR}expo_impo_rubros_usos.csv",
...
df.insert(3, "rubro_uso_corto", df.rubro_uso.apply(
    lambda s: s[s.find("(")+1:s.find(")")] if s != "Resto" else "Resto"))
...
def genera_uso_rubro_acumulada(df, importacion: bool):
    ...
    df_separada = df_separada.groupby(
        ["comercio", "anio", "rubro_uso", "rubro_uso_corto"], as_index=False).sum()
    df_separada["Participación porcentual"] = df_separada['valor'] / \
...
def genera_uso_rubro_agrupada():
    ...
    df_mensual = df_mensual[(df_mensual.anio == ultimo_anio) & (
        df_mensual.mes == ultimo_mes)].reset_index(drop=True)
    ...
    df_acumulada = df_acumulada[(
        df_acumulada.anio == ultimo_anio)].reset_index(drop=True)
...
def genera_var(df):
    ...
    impo_t_0 = (df[df.parent == "Importaciones"].value /
                (1+df[df.parent == "Importaciones"]["Variación porcentual"])).sum()
    impo_t = df[df.id == "Importaciones"].value.values[0]
    var_impo = impo_t/impo_t_0-1
    expo_t_0 = (df[df.parent == "Exportaciones"].value /
                (1+df[df.parent == "Exportaciones"]["Variación porcentual"])).sum()
...
def build_hierarchical_dataframe(df, levels, value_column, color_columns=None):
    ...
    # hay problema para concat una serie
    ...
    prop_expo_impo = pd.Series([df_all_trees.value.iloc[-3]/total,
                               df_all_trees.value.iloc[-2]/total, 1])  # un uno para rellenar "ica"
    df_all_trees["Participación porcentual"].iloc[-3:] = prop_expo_impo
    # va a salir una advertencia horrible
    df_all_trees.rubro_uso.iloc[-3:] = df_all_trees.id.iloc[-3:]
...
def plot_sunburst_usos_rubros(acumulado: bool):
    ...
    socios_sas_mensual_tree = build_hierarchical_dataframe(df=df, levels=[
                                                           "rubro_uso_corto", "comercio"], value_column="valor", color_columns="valor")
...
def plot_barras_usos_rubros(importacion: bool, acumulado: bool):
    ...
    if acumulado:
        ...
        df_rubro_uso = genera_uso_rubro_mensual(df, importacion=importacion)[genera_uso_rubro_mensual(
            df, importacion=importacion).mes == ultimo_mes].reset_index(drop=True)
    ...
    fig = go.Figure(data=[
        go.Bar(name=custom_data_1, x=df_rubro_uso[df_rubro_uso.anio == ultimo_anio-1].rubro_uso_corto, y=df_rubro_uso[df_rubro_uso.anio == ultimo_anio-1].valor,
               customdata=np.transpose([[custom_data_1]*len(eje_x),
                                        eje_x,
                                        df_rubro_uso[df_rubro_uso.anio ==
                                                     ultimo_anio-1]["Participación porcentual"],
        ...
        go.Bar(name=custom_data_2, x=df_rubro_uso[df_rubro_uso.anio == ultimo_anio].rubro_uso_corto, y=df_rubro_uso[df_rubro_uso.anio == ultimo_anio].valor,
               customdata=np.transpose([[custom_data_2]*len(eje_x),
                                        eje_x,
                                        df_rubro_uso[df_rubro_uso.anio ==
                                                     ultimo_anio]["Participación porcentual"],
                                        df_rubro_uso[df_rubro_uso.anio ==
                                                     ultimo_anio]["Variación porcentual"],
                                        ]),
               hovertemplate="<b>%{customdata[0]}</b><br><extra></extra>" +
               "%{customdata[1]}<br>Dólares $%{y:,.1f}<br>Participación porcentual: %{customdata[2]:.1%}" +
...
def genera_datos_barra_index():
    ...
    prueba_mensual = prueba_mensual[["comercio", "anio", "valor"]].groupby(
        ["comercio", "anio"], as_index=False).sum()
    prueba_mensual["variacion"] = prueba_mensual.groupby(
        ['comercio'])["valor"].pct_change(1, fill_method=None)
    prueba_mensual = prueba_mensual[prueba_mensual.anio == ultimo_anio].reset_index(
        drop=True)
    prueba_mensual["proporcion"] = prueba_mensual.valor / \
        prueba_mensual.valor.sum()

    prueba_acumulada = df.copy()
    prueba_acumulada = prueba_acumulada[["comercio", "anio", "valor"]].groupby(
        ["comercio", "anio"], as_index=False).sum()
    prueba_acumulada["variacion"] = prueba_acumulada.groupby(
        ['comercio'])["valor"].pct_change(1, fill_method=None)
    prueba_acumulada = prueba_acumulada[prueba_acumulada.anio == ultimo_anio].reset_index(
        drop=True)
    prueba_acumulada["proporcion"] = prueba_acumulada.valor / \
