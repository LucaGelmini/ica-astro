...
from modules.config.config import DIR_CUADROS, DIR_DASHBOARS
...
from modules.rubros_usos import plot_barras_usos_rubros, plot_sunburst_usos_rubros,genera_datos_barra_index
import json
...
cuadro5.to_json(f"{DIR_CUADROS}cuadro5.json",force_ascii=False, orient='table')

cuadro6.to_json(f"{DIR_CUADROS}cuadro6.json",force_ascii=False, orient='table')

df_grafico2_tabla.to_json(f"{DIR_CUADROS}df_grafico2_tabla.json", force_ascii=False, orient='table')

c1.to_json(f"{DIR_CUADROS}c1.json", force_ascii=False, orient='table')

cuadro7.to_json(f"{DIR_CUADROS}cuadro7.json",force_ascii=False, orient='table')

cuadro8.to_json(f"{DIR_CUADROS}cuadro8.json",force_ascii=False,orient='table')

r2_df.to_json(f"{DIR_CUADROS}r2_df.json",force_ascii=False, orient='table')

c2_expo.to_json(f"{DIR_CUADROS}c2_expo.json",force_ascii=False, orient='table')
c2_impo.to_json(f"{DIR_CUADROS}c2_impo.json",force_ascii=False, orient='table')

balanza.to_json(f"{DIR_CUADROS}balanza.json",force_ascii=False, orient='table')

desest_tabla_expo.to_json(f"{DIR_CUADROS}tabla desestacionalizada expo.json", force_ascii=False, orient='table')

desest_tabla_impo.to_json(f"{DIR_CUADROS}tabla desestacionalizada impo.json", force_ascii=False, orient='table')

tablas_socios[0].to_json(f"{DIR_CUADROS}socios_impo_mensual.json",force_ascii=False, orient='table')
tablas_socios[1].to_json(f"{DIR_CUADROS}socios_impo_acumulado.json",force_ascii=False, orient='table')
tablas_socios[2].to_json(f"{DIR_CUADROS}socios_expo_mensual.json" ,force_ascii=False, orient='table')
tablas_socios[3].to_json(f"{DIR_CUADROS}socios_expo_acumulado.json",force_ascii=False, orient='table')

# Datos para Dashboards

with open(f"{DIR_DASHBOARS}datos_barra_cajas.json","w") as f:
    json.dump(genera_datos_barra_index(),f)
